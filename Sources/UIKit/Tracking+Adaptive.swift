...
extension Tracking: AdaptiveStyleTransformation {
    ...
    func adapt(attributes theAttributes: StyleAttributes, to traitCollection: UITraitCollection) -> StyleAttributes? {
        if case .adobe = self {
            ...
            let styledFont = theAttributes[NSAttributedStringKey.font.rawValue] as? UIFont
            attributes.update(possibleValue: kerning(forFont: styledFont), forKey: NSAttributedStringKey.kern.rawValue)
            return attributes
        }
        else {
            return nil
